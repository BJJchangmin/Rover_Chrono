...
%Tunning factor
c_1 = 0.7; % Effect W,DP,T
c_2 = 0.1;   % Only Effect DP

n0 = 1; % Don't Touch. Only Effect Normal Force
n1 = 0.5;  % Don't Touch. Only Effect Normal Force
...
I_DP = (-(A.*A + B.*B)./(r_s.*A.*C).*T./60 + B.*W./A./60);

sigma_m = ks.*(r.*(cos(th_m)-cos(th_1))).^(n0+n1.*s);

D_exp = -r_s.*( (th_1_n-th_m) - (1-s).*(sin(th_1_n)-sin(th_m)))./janosi_k;
